...
def generate_random_string(length):
    ...
    characters = string.ascii_letters + string.digits  # You can customize this based on your needs
...
for p in range (0,phonebook_to_generate):
    ...
    with open(f"p{p+1}.txt","a") as phonebook:
        for contact in range(0,entry_per_phonebook):
            phonebook.write(f"{generate_random_string(random.randint(4,15))},{random.randint(1111111111,1999999999)}")
            phonebook.write('\n')
